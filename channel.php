...
if (count($check_channel)) {
	for ($i = 0; $i < count($check_channel); $i++) {
		...
		if (isset($_POST['subscribe'])) {
			...
			if ($loggedIn) {
				if ($created_by == $user) {
					header("Location: /channel/$channel_name");
				}
				if ($count_s > 0) {
		...
		<div class='channeloptions'>
			...
			<center>
				<img alt="" src='data/channels/images/icons/<?php echo $channel_icon; ?>' height='140' width='140' />
				<br /><br />
				<form action='/channel/<?php echo $channel_name; ?>' method='POST'>
		...
		<div class='channelvideocontainer'>
			...
			?>
			<!--<img alt="" src='#' height='100' width='180' />-->
		</div>
		<?php
	}
} else {
